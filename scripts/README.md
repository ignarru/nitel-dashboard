...
La tabla `leads_nitel` la **define el workflow de n8n de Nitel**, no este dashboard. El
...
| `setup-notify.sql` | Función + triggers `NOTIFY` sobre `leads_nitel` para el stream SSE en tiempo real. |
| `setup-respuestas.sql` | Agrega a `leads_nitel` las columnas `respuesta_texto` y `respuesta_recibida_at` + índice. |
| `setup-correo-html.sql` | Agrega a `leads_nitel` las columnas `correo_1_html`, `correo_2_html`, `correo_3_html`: guardan el HTML con formato (negritas, links, listas) que edita el operador en el dashboard. n8n nunca las toca. |
